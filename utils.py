        ...
        if sem_type == "linear-gauss":
            ...
            if linear_type == "linear":
                ...
                X[:, j, 0] = (
                    2.0 * np.sin(eta)
                    + eta
                    + np.random.exponential(scale=noise_scale, size=n)
                )
...
def simulate_sem(
    G: nx.DiGraph, n: int, sem_type: str, noise_scale: float = 1.0
) -> np.ndarray:
    """Simulate samples from SEM with specified type of noise.
...
def simulate_sem_multid(
    G: nx.DiGraph, n: int, x_dims: int, sem_type: str, noise_scale: float = 1.0
) -> np.ndarray:
    """Simulate samples from SEM with specified type of noise.
...
def count_accuracy_new(
    ...
    shd = len(extra_lower) + len(missing_lower) + len(reverse)
    try:
        sid = float(cdt.metrics.SID(target=B_true, pred=B))
    except Exception:
        sid = 0
    print(
...
def print_to_file(
    ...
    print_string = (
        "Mean SID: "
        + str(np.mean(result_sid))
        + "; std +-"
        + str(np.std(result_sid).item() / n)
    )
    f.write(print_string + "\n")
    print_string = (
        "Mean nnz: "
        + str(np.mean(result_nnz))
        + "; std +-"
        + str(np.std(result_nnz).item() / n)
    )
    print_string = (
        "Mean tpr: "
        + str(np.mean(result_tpr))
        + "; std +-"
        + str(np.std(result_tpr).item() / n)
    )
    print_string = (
        "Mean fpr: "
        + str(np.mean(result_fpr))
        + "; std +-"
        + str(np.std(result_fpr).item() / n)
    )
    print_string = (
        "Mean fdr: "
        + str(np.mean(result_fdr))
        + "; std +-"
