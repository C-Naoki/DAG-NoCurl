...
class BPR:
    ...
    def fit_cam(self, X):
        ...
        model = cdt.causality.graph.CAM(
            ...
            njobs=None,
            verbose=None,
        )  # causal additive model: Guasisan process + additive noise
    ...
    def fit_aug_lagr_AL2proj(self, X, hTol, lambda1, lambda2, threshold):
        ...
        def _func(w, choice="relu"):
            ...
            W = np.multiply(
                w1, relu(np.matmul(ones, phi.T) - np.matmul(phi, ones.T), False)
            )
            loss += _loss(W)
        ...
        def _grad(w, choice="relu"):
            ...
            # loss to W
            dF_dW_2 = np.multiply(
                W_grad, relu(np.matmul(ones, phi.T) - np.matmul(phi, ones.T), False)
            )
            ...
            # loss to phi
            dF_dPhi_2 = np.multiply(
                np.multiply(W_grad, w1),
                relu(np.matmul(ones, phi.T) - np.matmul(phi, ones.T), True),
            )
        ...
        prew = np.zeros(d * d)
        prebnds = [
            (0, 0) if i == j else (None, None) for i in range(d) for j in range(d)
        ]

        presol = sopt.minimize(
            _prefunc,
            prew,
            method="L-BFGS-B",
            jac=_pregrad,
    ...
    def fit_all_L2proj(self, X):
        Wstar, lossW = self.fit_aug_lagr_AL2proj(
            X,
            hTol=self.h_tol,
        ...
        return Wstar, [0.0], [num_non_zero], [lossW]
    ...
    def fit_all(self, X):
        ...
        for t in range(T):
            while rho < self.rho_max:
                sol = sopt.minimize(
                    _func, w_est, method="L-BFGS-B", jac=_grad, bounds=bnds
                )
                w_new = sol.x
                h_new = _h(w_new)
                if h_new > 0.25 * h:
...
def sigmoid(x, derivative=False):
    D = 10.0
    sigm = 1.0 / (1.0 + np.exp(-D * (x)))
    if derivative:
        return D * sigm * (1.0 - sigm)
    return sigm


def softrelu(x, derivative=False, alpha=0.1):
    D = 10.0
    rel = np.log((1.0 + np.exp((D * (x + 0.0))))) / D  # x * (x > 0)
    if derivative:
        return 1.0 / (1.0 + np.exp(-(D * (x + 0.0))))  # (x > 0)*1
    return rel
