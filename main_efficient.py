...
def get_args():
    ...
    parser.add_argument(
        ...
        choices=["linear-gauss", "linear-gumbel", "linear-laplace"],
    ...
    parser.add_argument(
        "--lambda1",
        type=float,
        default=1000.0,  # corresponding to lambda2
    ...
    parser.add_argument(
        "--rho_A_max",
        type=float,
        default=1e16,  # corresponding to rho, needs to  be >> lambda
    ...
    parser.add_argument(
        "--generate_data", type=int, default=1, help="generate new data or use old data"
    )
...
def main(args):
    ...
    d = args.data_variable_size  # number of variables
    ...
    result_sid = np.zeros((num_trials, 1))
    ...
    # repeat_write = [10, 20, 50, 100]  # seed numbers list to save results
    repeat_write = [100]
    repeat_counter = 0
    ...
    for trial_index in tqdm(range(num_trials)):
        file_name = (
            "./data/lineardata/"
            + str(args.data_sample_size)
            + "_"
            + str(args.data_variable_size)
            + "_"
            + str(args.graph_type)
            + "_"
            + str(args.graph_degree)
            + "_"
            + str(args.graph_sem_type)
            + "_"
            + str(trial_index)
            + ".pkl"
        )
        ...
        if args.data_type.startswith("nonlinear"):
            ...
            G = list(nx.DiGraph(graph))
        elif args.generate_data and not os.path.exists(file_name):
            G = list(utils.simulate_random_dag(d, degree, graph_type))
        ...
        # evaluate
        fdr, tpr, fpr, shd, sid, nnz, extra, missing, reverse = (
            [],
            [],
            [],
            [],
        ...
        for _G in G:
            _fdr, _tpr, _fpr, _shd, _sid, _nnz, _extra, _missing, _reverse = (
                utils.count_accuracy_new(_G, G_est)
            )
            fdr.append(_fdr)
            tpr.append(_tpr)
            fpr.append(_fpr)
        ...
        logger.info(
            "Accuracy: fdr %f, tpr %f, fpr %f, shd %d, sid %d, nnz %d",
            np.mean(fdr),
            np.mean(tpr),
            np.mean(fpr),
            np.mean(shd),
        ...
        result_sid[trial_index] = np.mean(sid)
        result_shd[trial_index] = np.mean(shd)
        result_nnz[trial_index] = np.mean(nnz)
        result_tpr[trial_index] = np.mean(tpr)
        result_fpr[trial_index] = np.mean(fpr)
        result_fdr[trial_index] = np.mean(fdr)
        # result_loss[trial_index] = (
        #     rho[-1] - loss_A_ground_truth[0]
        # )  # offset by ground truth

        result_extra[trial_index] = np.mean(extra)
        result_missing[trial_index] = np.mean(missing)
        result_reverse[trial_index] = np.mean(reverse)
    ...
    logger.info(
        "Accuracy: fdr "
        + str(np.mean(result_fdr).item())
        + "$\pm$"
        + str(np.std(result_fdr).item())
        + ", tpr "
        + str(np.mean(result_tpr).item())
        + "$\pm$"
        + str(np.std(result_tpr).item())
        + ", fpr "
        + str(np.mean(result_fpr).item())
        + "$\pm$"
        + str(np.std(result_fpr).item())
        + ", h "
        + str(np.mean(result_h).item())
        + "$\pm$"
        + str(np.std(result_h).item())
        + ", lossW "
        + str(np.mean(result_loss).item())
        + "$\pm$"
        + str(np.std(result_loss).item())
        + ", shd "
        + str(np.mean(result_shd).item())
        + "$\pm$"
        + str(np.std(result_shd).item())
        + ", sid "
        + str(np.mean(result_sid).item())
        + "$\pm$"
        + str(np.std(result_sid).item())
        + ", nnz "
        + str(np.mean(result_nnz).item())
        + "$\pm$"
        + str(np.std(result_nnz).item())
        + ", time "
        + str(np.mean(result_time).item())
        + "$\pm$"
    ...
    logger.info(
        "Edges: extra "
        + str(np.mean(result_extra).item())
        + "$\pm$"
        + str(np.std(result_extra).item())
        + ", missing "
        + str(np.mean(result_missing).item())
        + "$\pm$"
        + str(np.std(result_missing).item())
        + ", reverse "
        + str(np.mean(result_reverse).item())
        + "$\pm$"
        + str(np.std(result_reverse).item())
        + ", original nnz"
        + str(np.mean(result_originalW_nnz).item())
        + "$\pm$"
        + str(np.std(result_originalW_nnz).item())
    )
